...
_TYPES = Literal["hangman", "number guessing", "rock paper scissors", "simon", "impossible quiz", "tic tac toe", "cookie clicker"]
...
def save_score(username: str, game: _TYPES, score: int):
    data = read_csv(PATH)
    if data is None:
        data = pd.DataFrame(columns=["username", "hangman", "number guessing", "rock paper scissors", "simon", "impossible quiz", "tic tac toe", "cookie clicker"])
